...
@section('content')
  ...
  <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem;">
    <div>
      <h2 style="font-family: var(--font-display); font-size: 1.45rem; color: var(--eq-navy);">
        Products Portfolio & Atelier Stock
      </h2>
      <p style="font-size: 0.84rem; color: var(--eq-charcoal-soft); margin-top: 0.2rem;">
        Manage catalog items, pricing, inventory statuses, and creative showcase assets.
      </p>
    </div>
    <div>
      <a href="{{ route('admin.products.create') }}" class="eq-admin-btn eq-admin-btn--gold" style="padding: 0.6rem 1.25rem; font-size: 0.88rem; font-weight: 600; box-shadow: 0 2px 6px rgba(201, 150, 47, 0.3);">
          ...
          @forelse($products as $product)
            <tr>
              ...
              <td style="text-align: right;">
                <div style="display: inline-flex; align-items: center; justify-content: flex-end; gap: 0.45rem;">
                  
                  <!-- Toggle Stock Status Button -->
                  <form method="POST" action="{{ route('admin.products.toggle-stock', $product->id) }}" style="margin: 0;">
                    @csrf
                    @if($product->in_stock)
                      <button type="submit" class="eq-admin-btn eq-admin-btn--outline" style="padding: 0.35rem 0.65rem; font-size: 0.76rem; color: #b45309; border-color: #fde68a; background: #fffbeb;" title="Click to mark product Out of Stock">
                    ...
                    @else
                      <button type="submit" class="eq-admin-btn eq-admin-btn--success" style="padding: 0.35rem 0.65rem; font-size: 0.76rem;" title="Click to mark product In Stock">
                        <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                  ...
                  <!-- Edit Product Button -->
                  <a href="{{ route('admin.products.edit', $product->id) }}" class="eq-admin-btn eq-admin-btn--outline" style="padding: 0.35rem 0.65rem; font-size: 0.76rem;" title="Edit Product Details">
                    <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                  ...
                  <form method="POST" action="{{ route('admin.products.delete', $product->id) }}" style="margin: 0;" onsubmit="return confirm('Are you sure you want to permanently delete \'{{ addslashes($product->name) }}\'? This action cannot be undone.');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="eq-admin-btn eq-admin-btn--danger" style="padding: 0.35rem 0.65rem; font-size: 0.76rem;" title="Delete Product">
                      <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
